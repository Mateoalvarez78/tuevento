...
/**
 * Formatea un Date a "YYYY-MM-DD" usando SIEMPRE los getters locales
 * (getFullYear/getMonth/getDate), nunca toISOString() — toISOString()
 * convierte a UTC primero, y en Uruguay (UTC-3) eso corre la fecha un día
 * hacia adelante para cualquier hora local desde las 21:00 en adelante.
 * Única fuente de verdad para "hoy" (o cualquier Date) como string naive-local.
 */
export function localDateStr(date) {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
}

/** "YYYY-MM-DD" de hoy, en hora local (ver localDateStr). */
...
/**
 * Suma horas a una hora "HH:mm" y devuelve el horario resultante, cruzando
 * medianoche si corresponde. Solo para MOSTRAR un fin estimado (ej. paquetes
 * de duración fija de un salón) — no implica ningún bloqueo de disponibilidad.
 * `dayOffset` es 0 si termina el mismo día, 1 si cruza a la madrugada siguiente.
 */
export function addHoursToTime(time, hours) {
  const parsed = parseApiTime(time);
  if (!parsed || !Number.isFinite(hours)) return null;
