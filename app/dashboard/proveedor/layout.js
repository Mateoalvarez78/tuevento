    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center max-w-sm">
          ...
          <Button href="/provider/login">Ingresar</Button>
        </div>
      </div>
    );
  }
    ...
    <nav className="flex-1 overflow-y-auto px-3 py-3">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          ...
          >
            <AppIcon icon={item.icon} size={16} className={active ? 'text-primary' : 'text-gray-400 group-hover:text-gray-600'} aria-hidden="true" />
            ...
            {active && <AppIcon icon={ChevronRight} size={13} className="text-primary/50" aria-hidden="true" />}
          </Link>
        );
      })}
    </nav>
    ...
    <div className="px-4 pt-4 pb-3 border-b border-gray-100">
      <div className="flex items-center gap-3 bg-gray-50 rounded-2xl p-3">
        <div className="relative shrink-0">
          <Avatar src={user?.avatar} name={ownerName} size="md" shape="square" />
        ...
        <div className="flex-1 min-w-0">
          ...
          {providerData?.rating > 0 && (
            <div className="flex items-center gap-1 mt-1">
              <AppIcon icon={Star} size={10} className="text-yellow-400 fill-current" aria-hidden="true" />
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <aside className="hidden lg:flex w-72 bg-white border-r border-gray-100 flex-col fixed left-0 top-0 h-screen z-40">
          ...
          <div className="px-3 py-3 border-t border-gray-100">
            <Button variant="ghost" icon={Home} className="w-full !justify-start !text-gray-500 hover:!text-gray-700" href="/">
              Ir al inicio
            </Button>
            <Button
              variant="ghost"
              icon={LogOut}
              className="w-full !justify-start !text-red-500 hover:!bg-red-50 mt-0.5"
        ...
        <div className="lg:hidden">
          <Drawer open={open} onClose={() => setOpen(false)} side="left" width="max-w-72" bodyClassName="flex-1 flex flex-col overflow-y-auto">
        ...
        <div className="flex-1 lg:ml-72 flex flex-col min-h-screen">
          <header className="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sticky px-4 lg:px-6 py-3 flex items-center gap-3">
            <button onClick={() => setOpen(true)} className="lg:hidden p-1.5 -ml-1.5 rounded-lg hover:bg-gray-100 text-gray-500 shrink-0" aria-label="Abrir menú">
              <Avatar src={user?.avatar} name={ownerName} size="sm" />
            </button>
            ...
            {pendingCount > 0 && isAccountActive && (
              <div className="relative">
                ...
                />
                <span className="absolute top-0.5 right-0.5 w-4 h-4 bg-primary text-white text-[9px] font-bold rounded-full flex items-center justify-center pointer-events-none">
                  {pendingCount}
                </span>
              </div>
            )}
            ...
            <div className="hidden lg:flex items-center gap-2.5 bg-gray-50 rounded-xl px-3 py-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 flex items-center gap-1">
                  <AppIcon icon={BadgeCheck} size={10} className="text-primary" aria-hidden="true" /> {isAccountActive ? 'Verificado' : 'Proveedor'}
                </p>
              </div>
            </div>
          </header>
  ...
  return (
    <div className={`border-b ${cfg.bg} ${cfg.border} px-4 lg:px-6 py-2.5`}>
      <div className="flex items-start sm:items-center gap-2 text-sm flex-wrap">
        <AppIcon icon={cfg.icon} size={16} className={`shrink-0 ${cfg.text}`} aria-hidden="true" />
