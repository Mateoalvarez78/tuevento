...
function RegistroContent() {
  ...
  const [role, setRole] = useState(searchParams.get('tipo') === 'proveedor' ? 'provider' : '');
    ...
    <div className="min-h-screen bg-surface flex">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          {step === 0 && (
            <>
              <h1 className="text-2xl font-extrabold text-gray-900 mb-2">Crear cuenta gratis</h1>
              <p className="text-gray-500 mb-8 text-sm">¿Qué tipo de cuenta querés crear?</p>
              <div className="grid grid-cols-1 gap-4">
                <button
                  onClick={() => { setRole('client'); setStep(1); }}
                  className="group flex items-center gap-4 p-5 rounded-2xl border-2 border-gray-200 hover:border-primary hover:bg-primary-light transition-all text-left"
                >
                  <div className="w-12 h-12 rounded-2xl bg-blue-100 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors">
                    <User size={22} className="text-blue-600 group-hover:text-white" />
                  </div>
                  <div>
                    <div className="font-bold text-gray-900">Soy cliente</div>
                    <div className="text-sm text-gray-500">Quiero buscar y contratar servicios para mi evento.</div>
                  </div>
                </button>
                <button
                  onClick={() => { setRole('provider'); setStep(1); }}
                  className="group flex items-center gap-4 p-5 rounded-2xl border-2 border-gray-200 hover:border-primary hover:bg-primary-light transition-all text-left"
                >
                  <div className="w-12 h-12 rounded-2xl bg-orange-100 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors">
                    <Briefcase size={22} className="text-orange-600 group-hover:text-white" />
                  </div>
                  <div>
                    <div className="font-bold text-gray-900">Soy proveedor</div>
                    <div className="text-sm text-gray-500">Quiero publicar mis servicios y conseguir clientes.</div>
                  </div>
                </button>
              </div>
          ...
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                <button onClick={() => setStep(0)} className="text-gray-400 hover:text-gray-600 transition-colors text-xl leading-none">←</button>
                <div>
                  <h1 className="text-2xl font-extrabold text-gray-900">
                    {role === 'provider' ? 'Cuenta de proveedor' : 'Cuenta de cliente'}
                  </h1>
